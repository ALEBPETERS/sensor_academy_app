...
import { 
  ...
  ChevronRight, Calendar, User, Eye, ArrowLeft, Volume2, Sparkles, Check
} from "lucide-react";
...
export default function App() {
  ...
  const handleAuthSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!authEmail.trim() || !authFullName.trim()) return;
  ...
  // Bookmarking flow
  const handleToggleBookmark = (category: "papers" | "videos" | "books", id: string) => {
    if (!currentUser) {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {books.filter((b) => b.subject === selectedSubject.name).length === 0 ? (
                    ...
                    books.filter((b) => b.subject === selectedSubject.name).map((b) => (
                      <div key={b.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex gap-3 hover:border-slate-755 transition-colors">
          ...
          <div>
            ...
            {activeTab === "quizzes" && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {quizzes.map((q) => (
                    <div key={q.id} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 text-left hover:border-slate-705 transition-colors flex flex-col justify-between group space-y-5">
                      <div className="space-y-1.5">
                        ...
                        <p className="text-xs text-slate-450 leading-relaxed">This quiz features **{q.questions.length} multiple-choice question sets** with an assessment time allocation of **{q.durationMinutes} minutes**.</p>
                      </div>
            ...
            {activeTab === "admin" && currentUser?.isAdmin && (
              <AdminPanel
                ...
                allBooks={books}
                allQuizzes={quizzes}
                allStudents={[currentUser]} // Preload registering list
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5 text-xs">
                ...
                <select
                  ...
                  className="w-full bg-slate-955 border border-slate-800 p-3 rounded-xl text-white outline-none"
                >
