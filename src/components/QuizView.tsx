...
import { Clock, Check, X, AlertCircle, Award, RotateCcw, ArrowRight, ArrowLeft, RefreshCw, BarChart } from "lucide-react";
...
export default function QuizView({ quiz, onQuizSubmit, onBack }: QuizViewProps) {
  ...
  const handleAutoSubmit = () => {
    // Collect and submit automatically on timer timeout
    submitCompletedQuiz();
  };
  ...
  return (
    <div id="quiz-workspace" className="bg-slate-950 min-h-screen text-slate-150 pb-20 pt-6 px-4">
        ...
        {quizState === "intro" && (
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-10 shadow-2xl text-center space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-4">
              <button
                onClick={onBack}
                className="w-full sm:w-auto px-6 py-2.5 bg-slate-855 text-slate-350 hover:text-white hover:bg-slate-800 border border-slate-700/50 rounded-xl font-semibold transition-all active:scale-95 text-xs"
              >
                Back to Tests
              </button>
              <button
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex items-center justify-between shadow-lg">
              ...
              <div className={`flex items-center space-x-2 py-1.5 px-3 rounded-full border transition-colors ${
                ...
                <span className="font-mono text-xs font-bold font-semibold">{formatTime(timeLeft)}</span>
              </div>
            </div>
                    ...
                    <button
                      ...
                      className={`w-full p-4 rounded-2xl border text-left flex items-center transition-all focus:outline-none ${
                        isSelected
                          ...
                          : "bg-slate-950/60 border-slate-800 text-slate-350 hover:bg-slate-800/45 hover:border-slate-700"
                      }`}
                    >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-10 shadow-2xl text-center space-y-5">
              ...
              <div className="flex flex-col items-center py-4">
                ...
                <p className="text-sm font-semibold text-slate-350 mt-3">
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                <button
                  onClick={onBack}
                  className="px-6 py-2.5 bg-slate-855 hover:bg-slate-800 border border-slate-700 hover:text-white rounded-xl text-xs font-semibold text-slate-350 transition-all active:scale-95 flex items-center space-x-1.5"
                >
                  ...
                  <div key={q.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3.5 shadow-md">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                      <div className={`p-3 rounded-xl border ${
                        ...
                        <span className="font-semibold text-slate-150 block mt-0.5">
                      ...
                      <div className="p-3 bg-emerald-950/25 border border-emerald-500/20 rounded-xl">
                        <span className="text-emerald-450 block font-mono">CORRECT CHOICE</span>
                        <span className="font-semibold text-slate-150 block mt-0.5">
                    ...
                    {q.explanation && (
                      <div className="bg-slate-950 p-3.5 border border-slate-805 rounded-xl text-xs space-y-1">
                        <span className="text-blue-400 font-bold uppercase block tracking-wide">Study Explanation Note:</span>
                        <p className="text-slate-350 leading-relaxed font-mono whitespace-pre-line">{q.explanation}</p>
                      </div>
                    )}
                  </div>
                );
